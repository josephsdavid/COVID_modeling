...
class SIR:
    def __init__(
        self, S: int, I: int, R: int, beta: float, gamma: float, beta_decay: float = 0
    ) -> None:
    ...
    def _step(self, S: int, I: int, R: int) -> Tuple[float, float, float]:
        Sn = self._Sn(S, I)
        Rn = self._Rn(I, R)
        In = self._In(S, I)
        ...
        I_float = In * scale
        R_float = Rn * scale
        return S_float, I_float, R_float

    def sir(self, n_days: int) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        S = self.S
        I = self.I
        R = self.R
        self.s = [S]
        ...
        for day in range(n_days):
            ...
            self.i.append(I)
            self.r.append(R)
        return tuple(np.asarray(x) for x in [self.s, self.i, self.r])
...
class SIRD(SIR):
    ...
    def _step(self, S: int, I: int, R: int) -> Tuple[float, float, float]:
        S_float, I_float, R_float = super()._step(S, I, R)
        return S_float, I_float, R_float

    def sir(self, n_days: int) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        S = self.S
        I = self.I
        R = self.R
        D = self.D
        ...
        # TODO: Supertype SIR returns only 3 values - the function signatures
        #       should match
        return tuple(np.asarray(x) for x in [self.s, self.i, self.r, self.d])
